"""
5×7 Bitmap Font for eInk Display
Compact monospace font, 5 columns × 7 rows (1 row baseline).
Each character is 5 bytes, stored column-major.
"""
...
FONT_WIDTH = 5
FONT_HEIGHT = 8  # 7 pixels + 1 baseline
FONT_SPACING = 1  # extra pixel between chars


def draw_char(display, x, y, ch, bw=0, red=0):
    """Draw a single character at (x, y) in specified color."""
    glyph = FONT_5X7.get(ord(ch), FONT_5X7[32])
    for col in range(FONT_WIDTH):
        bits = glyph[col]
        for row in range(7):
            if bits & (1 << row):
                display.pixel(x + col, y + row, bw, red)


def draw_text(display, x, y, text, bw=0, red=0):
    """Draw a text string at (x, y) in specified color."""
    cx = x
    for ch in text:
        if ch == '\n':
            cx = x
            ...
            continue
        if cx + FONT_WIDTH > display.width:
            cx = x
            y += FONT_HEIGHT + FONT_SPACING
        draw_char(display, cx, y, ch, bw, red)
        cx += FONT_WIDTH + FONT_SPACING


def draw_text_wrap(display, x, y, w, text, bw=0, red=0):
    """Draw text wrapped within width w, returning y position after last line."""
    chars_per_line = w // (FONT_WIDTH + FONT_SPACING)
    cx, cy = x, y
    word = ''
    for ch in text:
        if ch == '\n':
            draw_text(display, cx, cy, word, bw, red)
            cx += (len(word) * (FONT_WIDTH + FONT_SPACING))
            word = ''
            cx = x
            cy += FONT_HEIGHT + FONT_SPACING
        elif ch == ' ':
            if cx + (len(word) + 1) * (FONT_WIDTH + FONT_SPACING) > x + w:
                draw_text(display, cx, cy, word, bw, red)
                cx = x
                cy += FONT_HEIGHT + FONT_SPACING
                word = ''
                ...
                word = ''
                # space
                draw_text(display, cx, cy, ' ', bw, red)
                cx += FONT_WIDTH + FONT_SPACING
        else:
            word += ch
    if word:
